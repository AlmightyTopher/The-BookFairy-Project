import { Message, ActionRowBuilder, ButtonBuilder, ButtonStyle, ButtonInteraction, MessageFlags, EmbedBuilder } from 'discord.js';
...
// Removed createGenreSelectionScreen import - using flow engine instead
import { 
  renderHome, 
  navigateFromHome, 
  forwardLegacyToHome, 
...
*Bless your heart for askin' - now get to searchin'!* 📚`;
  ...
  async handleButtonInteraction(interaction: ButtonInteraction) {
    try {
      ...
      // Check for unified home hub navigation
      const homeNavigation = await navigateFromHome(interaction.customId, interaction);
      if (homeNavigation) {
        switch (homeNavigation.action) {
          case 'search_title':
          ...
          case 'search_describe':
            await interaction.reply({ 
              content: "📝 **Describe the Book**\nTell me what you remember about the book:", 
              flags: MessageFlags.Ephemeral 
            });
            return;
          ...
          case 'browse_genres':
            try {
              // Use flow engine for genre selection
              this.flowEngine.navigateTo(interaction.user.id, 'Genre.Pick');
              const rendered = this.flowEngine.renderRoute(interaction.user.id);
          ...
          case 'audiobooks':
            await interaction.reply({ 
              content: "🎧 **Audiobooks**\nSearching for popular audiobooks...", 
              flags: MessageFlags.Ephemeral 
            });
            ...
            try {
              ...
              if (result && 'results' in result && result.results.length > 0) {
                ...
                const responseMsg = `Found ${session.allResults?.length || validatedResponse.results.length} popular audiobooks:\n\n` +
                ...
                await interaction.followUp({ 
                  content: this.personality.transformMessage_test("Sorry sugar, I couldn't find any popular audiobooks right now. Try searching for something specific!", 'error')
                });
              }
            } catch (error) {
              logger.error({ error }, 'Failed to search for audiobooks');
              await interaction.followUp({ 
                content: this.personality.transformMessage_test("Oh mercy, somethin' went wrong while lookin' for audiobooks. Try again, darlin'!", 'error')
              });
            }
            return;
            
          case 'more_options':
            // Simulate clicking the "More Options" button to reach the existing screen
            const moreOptionsEmbed = new EmbedBuilder()
              .setTitle('📚 More Search Options')
              .setDescription('More options, pick a path or set filters.')
              .setColor(0x7C4DFF);
              
            const moreOptionsRow1 = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
            ...
            const moreOptionsRow2 = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
            ...
            const moreOptionsRow3 = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
            ...
            const moreOptionsAnchorRow = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
                  .setCustomId('home_new_chat')
                  .setLabel('New Chat')
            ...
            await interaction.reply({ 
              embeds: [moreOptionsEmbed],
              components: [moreOptionsRow1, moreOptionsRow2, moreOptionsRow3, moreOptionsAnchorRow],
              flags: MessageFlags.Ephemeral 
            });
            return;
            
          case 'other_commands':
            // Create Other Commands screen inline
            const otherCommandsEmbed = new EmbedBuilder()
              .setTitle('🔧 Other Commands')
              .setDescription('Other commands, pick a utility, or hit New Chat to start over.')
              .setColor(0x7C4DFF);

            const otherCommandsRow = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
            ...
            const otherCommandsAnchorRow = new ActionRowBuilder<ButtonBuilder>()
              .addComponents(
                new ButtonBuilder()
                  .setCustomId('home_new_chat')
                  .setLabel('New Chat')
            ...
            await interaction.reply({ 
              embeds: [otherCommandsEmbed],
              components: [otherCommandsRow, otherCommandsAnchorRow],
              flags: MessageFlags.Ephemeral 
            });
            return;
          ...
          case 'home_refresh':
          case 'home_new_chat':
            // Clear session and show fresh home
            if (homeNavigation.action === 'home_new_chat') {
              clearHomeSession(interaction.user.id);
              // Also clear legacy session
              session.allResults = [];
              session.currentPage = 0;
              session.hasShownResults = false;
              session.lastResponse = undefined;
              session.moreInfoMode = false;
            }
            
            const home = loadHome(interaction.user.id);
            await interaction.reply({ 
          ...
          case 'home_next_reserved':
            await interaction.reply({ 
              content: "This feature is reserved for future use. Stay tuned!", 
              flags: MessageFlags.Ephemeral 
            });
            return;
        }
      ...
      } else if (interaction.customId === 'other_cmds_open') {
        // Handle "Other Commands" button - show other commands screen
        const otherCommandsEmbed = new EmbedBuilder()
          .setTitle('🔧 Other Commands')
          .setDescription('Other commands, pick a utility, or hit New Chat to start over.')
          .setColor(0x7C4DFF);
  ...
  async handle(message: Message) {
    ...
    try {
      ...
      if (query.toLowerCase().match(/^(!?genres?|!?browse\s*genres?)$/)) {
        try {
          // Use flow engine for genre selection
          this.flowEngine.navigateTo(message.author.id, 'Genre.Pick');
          const rendered = this.flowEngine.renderRoute(message.author.id);
